...
/**
 * FingerprintList — now receives panelData from App.jsx.
 * No longer makes its own API call — data comes from the unified
 * /api/station-panel endpoint (single Gemini call for all cards).
 */
export default function FingerprintList({ panelData, panelLoading, lat, lng }) {
  ...
  const fingerprint = panelData.fingerprint || [];
  const explanation = panelData.attribution_explanation || '';
  ...
  return (
    <div className="bg-white p-5 rounded-xl border border-outline-variant shadow-soft">
      ...
      <div className="space-y-4 mb-4">
        {fingerprint.sort((a, b) => b.contribution_pct - a.contribution_pct).map((item, idx) => (
